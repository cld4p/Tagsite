...
execute if score @s iestimer matches 90.. run effect give @a[team=Blight,distance=..6] minecraft:glowing 10 0 true
...
execute if score @s iestimer matches 90.. run execute as @a at @s if score @s uid = @n[tag=noexist,type=item_display,tag=beam_ies_display] uid run tellraw @s {"text":"IES Triggered!","color":"blue"}
execute if score @s iestimer matches 90.. run execute as @a at @s if score @s uid = @n[tag=noexist,type=item_display,tag=beam_ies_display] uid run playsound block.amethyst_block.resonate master @s ~ ~ ~ 1 0.5
execute if score @s iestimer matches 90.. run kill @n[type=allay,tag=beam_ies]
